import { ArtiklarPage, BerattelsePage, OvningarPage, HalsaPage, CommunityPage, PriserPage, KontoPage } from "./Pages_tryggman";
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'0.5rem',padding:'0.5rem 0'}}>
                {SUGGESTIONS.map((s,i) => (
                  <button key={i} onClick={()=>sendMessage(s)}
                    style={{background:'rgba(58,125,110,0.12)',border:'1px solid rgba(58,125,110,0.3)',color:'rgba(255,255,255,0.7)',padding:'0.6rem 1rem',textAlign:'left',fontFamily:'DM Sans,sans-serif',fontSize:'0.85rem',cursor:'pointer'}}
                  >{s}</button>
                ))}
              </div>
            )}
          ...
          {isLocked ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="chat-input-bar">
                <input ref={inputRef} type="text" value={input} onChange={e=>setInput(e.target.value)} placeholder="Skriv här..." disabled={loading} className="chat-input" autoComplete="off" autoFocus={modalOpen}/>
